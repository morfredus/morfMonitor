...
Copie autonome de la bibliothèque partagée de morfredus utilisée par morfMonitor :

- `beacon/` — **morfBeacon** : supervision LAN (heartbeat UDP « je suis actif » +
  endpoint HTTP `/status`). morfMonitor s'en sert pour s'annoncer sur le réseau.

> morfMonitor n'embarque **que** morfBeacon (contrairement à ComponentHub, qui
